...
This will be demonstrated by using ORA and FCS approaches.
Clusterprofiler will be used for ORA, while CAMERA will be used for FCS.
...
This is a pre-ranked approximation for CAMERA.
...
dem <-merge(de,gt,by=0)
stat <- dem$stat
names(stat) <- dem$GeneSymbol
cres <- cameraPR(statistic=stat, index=genesets, use.ranks = TRUE, inter.gene.cor=0.01, sort = TRUE )
cs_up <- rownames(subset(cres,Direction=="Up" & FDR<0.05))
cs_dn <- rownames(subset(cres,Direction=="Down" & FDR<0.05))
message(paste("Number of up-regulated pathways:",length(cs_up) ))
message(paste("Number of down-regulated pathways:",length(cs_dn) ))
head(cres,10) #%>% kbl() %>% kable_paper("hover", full_width = F)
...
names(stat) <- dem$GeneSymbol
cres <- cameraPR(statistic=stat, index=genesets, use.ranks = TRUE, inter.gene.cor=0.01, sort = TRUE )
cc_up <- rownames(subset(cres,Direction=="Up" & FDR<0.05))
cc_dn <- rownames(subset(cres,Direction=="Down" & FDR<0.05))
message(paste("Number of up-regulated pathways:",length(cc_up) ))
message(paste("Number of down-regulated pathways:",length(cc_dn) ))
head(cres,10) #%>% kbl() %>% kable_paper("hover", full_width = F)
